...
class ContribucionBackendForm extends ContribucionFormUno {
    ...
    public function configure() {
        ...
        if (sfContext::getInstance()->getModuleName() == 'contribuciones_destacadas' && (sfContext::getInstance()->getRequest()->getParameter('action') == "new" || sfContext::getInstance()->getRequest()->getParameter('action') == "create")) {
        ...
        if ($pieces[2] == "contribucion_empresa") {
            ...
            $q = Doctrine_Query::create()->from('Concurso')->where('concurso_tipo_id = 2')->addWhere('concurso_estado_id = 2');
            $this->widgetSchema['concurso_id'] = new sfWidgetFormDoctrineChoice(array('model' => $this->getRelatedModelName('Concurso'), 'add_empty' => false, 'query' => $q, 'order_by' => array('id', 'desc')));
        } else {
            $q = Doctrine_Query::create()->from('Concurso')->where('concurso_estado_id = 2');
            $this->widgetSchema['concurso_id'] = new sfWidgetFormDoctrineChoice(array('model' => $this->getRelatedModelName('Concurso'), 'add_empty' => false, 'query' => $q, 'order_by' => array('id', 'desc')));
        }
