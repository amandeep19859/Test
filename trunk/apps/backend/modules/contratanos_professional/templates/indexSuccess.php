...
<div id="sf_admin_container">
    <h1><?php echo __('Listado de formularios de Contrátanos para Profesional', array(), 'messages') ?></h1>

    <?php include_partial('contratanos_professional/flashes') ?>

    <div id="sf_admin_header">
        <?php include_partial('contratanos_professional/list_header', array('pager' => $pager)) ?>
    </div>
    ...
    <div id="sf_admin_bar" class="filters_center" style="float: none;margin: auto;width: 400px;<?php echo (count($filtershow) <= 1) ? 'display: none;' : ''; ?>">
        <?php include_partial('contratanos_professional/filters', array('form' => $filters, 'configuration' => $configuration)) ?>
    </div>
    <?php if (count($filtershow) != 0): ?>
        <div class="clean clear clear_0" >&nbsp;</div>
    <?php endif; ?>
    <div id="sf_admin_content">
        <form action="<?php echo url_for('contratanos_contratanos_professional_collection', array('action' => 'batch')) ?>" method="post">
            <?php include_partial('contratanos_professional/list', array('pager' => $pager, 'sort' => $sort, 'helper' => $helper)) ?>
            <ul class="sf_admin_actions">
                <?php include_partial('contratanos_professional/list_batch_actions', array('helper' => $helper)) ?>
                <?php include_partial('contratanos_professional/list_actions', array('helper' => $helper)) ?>
            </ul>
        </form>
    </div>

    <div id="sf_admin_footer">
        <?php include_partial('contratanos_professional/list_footer', array('pager' => $pager)) ?>
    </div>
</div>
