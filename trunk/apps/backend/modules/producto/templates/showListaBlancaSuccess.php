        ...
        <div class="sf_admin_list">
            <ul class="dragbox-content">
                ...
                <?php if ($producto->getLista() == "lb"): ?>
                    <?php if ($producto->getRawValue()->getComentarioInicial()): ?>
                        <br/>
                        <li class="comment" style="margin-bottom: 5px;"><strong>Comentario inicial: </strong><p class="mr-span"> </p>
                            <div class="comenario_p">
                                <?php echo $producto->getRawValue()->getComentarioInicial() ?>
                            </div>
                            <div class="clean_clear_blanca"></div>
                            <span class="ver_link">                           
                                <?php echo link_to('ver +', 'producto/showComentarioInicial?id=' . $producto->getId(), array("popup" => array("popWindow", "width=650,height=500, left=200, scrollbars=1, menubar=1, scrollbars=1"))) ?>
            ...
            <?php if ($pager->haveToPaginate()): ?>
                <div class="pagination">
                    ...
                    <?php echo link_to(image_tag('/images/first.png', array('title' => 'Primero')), 'producto/show?page=' . $pager->getFirstPage() . '&pg=' . $sf_request->getParameter('pg') . '&id=' . $producto->id) ?>
                    <?php echo link_to(image_tag('/images/previous.png', array('title' => 'Anterior')), 'producto/show?page=' . $pager->getPreviousPage() . '&pg=' . $sf_request->getParameter('pg') . '&id=' . $producto->id) ?>
                    <?php foreach ($pager->getLinks() as $page): ?>
                        <?php echo ($page == $pager->getPage()) ? $page : link_to($page, 'producto/show?page=' . $page . '&pg=' . $sf_request->getParameter('pg', 1) . '&id=' . $producto->id) ?>
                        <?php if ($page != $pager->getCurrentMaxLink()): ?>
                            -
                        <?php endif ?>
                    <?php endforeach ?>
                    <?php echo link_to(image_tag('/images/next.png', array('title' => 'Siguiente')), 'producto/show?page=' . $pager->getNextPage() . '&pg=' . $sf_request->getParameter('pg') . '&id=' . $producto->id) ?>
                    <?php echo link_to(image_tag('/images/last.png', array('title' => 'Último')), 'producto/show?page=' . $pager->getLastPage() . '&pg=' . $sf_request->getParameter('pg') . '&id=' . $producto->id) ?>
                </div>
            ...
            <?php if ($pager_pending->haveToPaginate()): ?>
                <div class="pagination">
                    ...
                    <?php echo link_to(image_tag('/images/first.png', array('title' => 'Primero')), 'producto/show?pg=' . $pager_pending->getFirstPage() . '&page=' . $sf_request->getParameter('page') . '&id=' . $producto->id) ?>
                    <?php echo link_to(image_tag('/images/previous.png', array('title' => 'Anterior')), 'producto/show?pg=' . $pager_pending->getPreviousPage() . '&page=' . $sf_request->getParameter('page') . '&id=' . $producto->id) ?>
                    <?php foreach ($pager_pending->getLinks() as $page): ?>
                        <?php echo ($page == $pager_pending->getPage()) ? $page : link_to($page, 'producto/show?pg=' . $page . '&page=' . $sf_request->getParameter('page', 1) . '&id=' . $producto->id) ?>
                        <?php if ($page != $pager_pending->getCurrentMaxLink()): ?>
                            -
                        <?php endif ?>
                    <?php endforeach ?>
                    <?php echo link_to(image_tag('/images/next.png', array('title' => 'Siguiente')), 'producto/show?pg=' . $pager_pending->getNextPage() . '&page=' . $sf_request->getParameter('page') . '&id=' . $producto->id) ?>
                    <?php echo link_to(image_tag('/images/last.png', array('title' => 'Último')), 'producto/show?pg=' . $pager_pending->getLastPage() . '&page=' . $sf_request->getParameter('page') . '&id=' . $producto->id) ?>
                </div>
