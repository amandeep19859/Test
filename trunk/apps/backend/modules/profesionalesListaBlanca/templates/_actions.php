...
<?php if(in_array($profesional->profesional_estado_id,array(2,3))):?>
	...
	<?php if ($profesional->destacado):?>
			<li><?php echo image_tag('info-icon') ?><strong>¡Profesional destacado!</strong> <?php echo link_to("Quitar destacado","profesionales_pendientes/retirar?tipo=normal&profesional_id=".$profesional->id)?></li>
		<?php else:?>
			...
			<?php else:?>
				<li><?php echo link_to_function("Destacar profesional","alert('No puedes destacar más de 10 profesional a la vez')")?></li>
			<?php endif;?>
	<?php endif; ?>	
