...
<div class="pagination" style="text-align:right">
  <?php echo link_to_function(image_tag('/images/first.png',array('title'=>'Primera')),'$("#Concursos75dias_home_content").load("escritorio/get_concursos75dias?page='.$concursos->getFirstPage().'")') ?>	
  <?php echo link_to_function(image_tag('/images/previous.png',array('title'=>'Anterior')), '$("#Concursos75dias_home_content").load("escritorio/get_concursos75dias?page='.$concursos->getPreviousPage().'")') ?> 
  <?php $links = $concursos->getLinks(); foreach ($links as $page): ?>
    <?php echo ($page == $concursos->getPage()) ? $page : link_to_function($page, '$("#Concursos75dias_home_content").load("escritorio/get_concursos75dias?page='.$page.'")') ?>
    <?php if ($page != $concursos->getCurrentMaxLink()): ?> - <?php endif ?>
  <?php endforeach ?>
  <?php echo link_to_function(image_tag('/images/next.png',array('title'=>'Siguiente')), '$("#Concursos75dias_home_content").load("escritorio/get_concursos75dias?page='.$concursos->getNextPage().'")') ?> 
  <?php echo link_to_function(image_tag('/images/last.png',array('title'=>'Última')), '$("#Concursos75dias_home_content").load("escritorio/get_concursos75dias?page='.$concursos->getLastPage().'")') ?>
</div>
<?php endif ?>
