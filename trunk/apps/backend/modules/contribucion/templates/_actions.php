...
<ul>
    <?php if ($contribucion->contribucion_estado_id == 1): ?>
        <?php if (sfContext::getInstance()->getModuleName() != 'concurso'): ?>
            <li><?php echo link_to_function(image_tag("tick") . "Activar", 'procesar_puntos()')//link_to(image_tag("tick")."Poner como activa","contribucion/changeStatus?id=".$contribucion->id."&estado=2")                        ?></li>
        ...
        <?php endif; ?>
        <li><?php echo link_to(image_tag("email") . "Sugerir Modificaciones", "contribucion/rechazar?id=" . $contribucion->id) ?></li>
    <?php endif; ?>
</ul>
...
<?php if ($contribucion->contribucion_estado_id == 2): ?>
    <ul>
        <?php if ($contribucion->destacado): ?>
            ...
            <?php if ($module_name == 'concurso'): ?>
                <li><?php echo link_to("Quitar destacado", "concurso/retirarcuncurso?concurso_id=" . $sf_request->getParameter('id') . "&contribucion_id=" . $contribucion->id) ?></li>
            <?php else: ?>
                <li><?php echo link_to("Quitar destacado", "contribucion/retirar?contribucion_id=" . $contribucion->id) ?></li>
            <?php endif; ?>
        <?php else:
            ?>
            <?php if ($n_contribuciones_destacados < 10): ?>
                <?php if ($module_name == 'planes_de_accion' || $module_name == 'planes_de_accion_empresa' || $module_name == 'planes_de_accion_producto'): ?>
                    <li><?php echo link_to("Destacar contribucion", "contribucion/destacar?plan_de_accion=plan&contribucion_id=" . $contribucion->id) ?></li>
                <?php else: ?>
                    <li><?php echo link_to("Destacar contribucion", "contribucion/destacar?contribucion_id=" . $contribucion->id) ?></li>
                <?php endif; ?>
            <?php else: ?>
...
<ul>
    <li class="sf_admin_action_edit"><?php echo link_to("Editar contribucion", "contribucion/edit?id=" . $contribucion->id) ?></li>
