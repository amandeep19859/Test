...
<ul>
    <?php if ($contribucion->contribucion_estado_id == 1): ?>
        ...
        <li><?php echo link_to(image_tag("email") . "Sugerir Modificaciones", "contribuciones_destacadas/rechazar?id=" . $contribucion->id) ?></li>
    <?php endif; ?>
</ul>

<?php if ($contribucion->contribucion_estado_id == 2 && in_array($contribucion->getConcurso()->getConcursoEstadoId(), array(2, 3))): ?>
    <ul>	
        <?php if ($contribucion->destacado): ?>
            <li><?php echo image_tag('/images/info-icon.png') ?><strong>¡Contribucion destacada!</strong></li>
            <li><?php echo link_to("Quitar destacado", "contribuciones_destacadas/retirar?contribucion_id=" . $contribucion->id) ?></li>
        <?php else: ?>
            <?php if ($n_contribuciones_destacados < 10): ?>
                <li><?php echo link_to("Destacar contribucion", "contribuciones_destacadas/destacar?contribucion_id=" . $contribucion->id) ?></li>
            <?php else: ?>
...
<ul>
    <li class="sf_admin_action_edit"><?php echo link_to("Editar contribucion", "contribuciones_destacadas/edit?id=" . $contribucion->id) ?></li>
    <?php echo $helper->linkToDelete($contribucion, array('params' => array(), 'confirm' => 'Are you sure?', 'class_suffix' => 'delete', 'label' => 'Delete',)) ?>
    <li> <?php echo link_to('Volver a contribuciones destacadas', '@contribuciones_destacadas') ?></li>
    <li><?php echo link_to('Volver a concurso', 'concurso/show?id=' . $contribucion->getConcursoId()) ?></li>
</ul>
