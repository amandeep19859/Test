...
<script type='text/javascript'>
    function disableSectorTres(id) {
        if ($('#' + id + '  option').size() <= 1) {
            ...
            if ($('#lista_cuestionario_tipo').val() == 'empresa') {
                $('#' + id).append('<option value="">Selecciona actividad</option>');
            } else {
                $('#' + id).append('<option value="">Selecciona tipo de producto</option>');
    ...
    $('#lista_cuestionario_empresa_sector_dos_id').change(function(){
        if($('#lista_cuestionario_empresa_sector_tres_id option').size() == 1){
            $('#lista_cuestionario_empresa_sector_tres_id').attr('disabled','disabled');
        }
    });

    $('#lista_cuestionario_empresa_sector_dos_id').each(function(){
        //alert($('#concurso_producto_tipo_dos_id option:selected').val());
        if($('#lista_cuestionario_empresa_sector_dos_id option:selected').val()){
            if($('#lista_cuestionario_empresa_sector_tres_id option').size() == 1){
                $('#lista_cuestionario_empresa_sector_tres_id').attr('disabled','disabled');
            }
        }
...
<style type="text/css">
    ...
    label#Preguntas{font-weight: bold !important;}
    ul.inline{margin: 0 13px;}
