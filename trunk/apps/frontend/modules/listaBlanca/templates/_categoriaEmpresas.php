<div id="menu">
    <ul class="lista-categorias accordion" rel='<?php echo url_for($url) ?>'>
        <?php
        $i = 0;
        foreach ($categoriasSectorUno as $categoria) : $i++
            ...
            <li class="<?php echo ($i == 0) ? 'first' : '' ?>">
                ...
                <ul>
                    <?php foreach ($categoria->getEmpresaSectorDos() as $categoria2) : ?>
                        <li><a href='#'  data-link="<?php echo url_for($url, array('sector1' => $categoria->getSlug(), 'sector2' => $categoria2->getSlug())) ?>"><?php echo $categoria2->getName() ?></a>

                            <?php if ($categoria2->getEmpresaSectorTres()->count() > 0) : ?>
                                <ul>
                                    <?php foreach ($categoria2->getEmpresaSectorTres() as $categoria3) : ?>
