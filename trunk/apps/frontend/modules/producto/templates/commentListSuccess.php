...
<script type="text/javascript">
  var url = "<?php echo '/product/commentRecord/' ?>";
  
  $(document).ready(function(){
    //when audit button is click
    $('.l-audita').bind('click', function(){
      var audit_id = $(this).data('id');
      var product_id = $(this).data('company');
      //request for audit records
      $.ajax({
        url: url+ 'product/'+ product_id+'/id/'+audit_id,
        success: function(data){
         $('#audit-list-' + product_id).html(data) ; 
         $('.a-list').addClass('hidden');
         $('#audit-list-' + product_id).removeClass('hidden');
        }
